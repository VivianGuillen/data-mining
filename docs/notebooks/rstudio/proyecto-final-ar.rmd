...
Escribe la fecha actual (**NOTE-01**): <04/06/2021>
...
```{r}
NOMBRE_COMPLETO = "Vivian Guillen"
DIGITOS_MATRICULA = 01746561
```
...
- ¿Cuál es la interpretación de la regla `{} => {diapers}` en la que se aprecia la ausencia del LHS? (**NOTE-19**):la interpretación sería que no existe una relación entre diapers con algún otro artículo.
...
- ¿Por qué no aparece listada la regla `{beer} => {nuts}`? (**NOTE-22**): Las metricas de confidence y support excluyen esta regla por lo que se elimina la aparición de esta regla.
...
- ¿Cuál es el propósito de `session id`? (**NOTE-23**): Es un identificador único de la sesión y como tal apoya a dar un servicio personalizado mediante recomendaciones adecuadas de acuerdo al historial de gustos, o música escuchada.
- ¿Cuál es el propósito de `track id`? (**NOTE-24**):Es el identificador único de cada track, lo que ayuda a tener registros de reproducción de dicho track en cierta cuenta, lo que ayuda a generar vínculos entre canciones parecidas y usuarios, lo que ayuda a brindar recomendaciones a los usuarios.
- ¿Consideras que las otras columnas del dataset son aplicables para análisis por medio de reglas de asociación, justifica tu respuesta? (**NOTE-25**): Sí, ya que los datos recopilados nos brindan información con la que se puede visualizar la actividad del usuario en la plataforma, lo que ayuda a generar relaciones entre la infromación para desarrollar las estrategias de recomendaciones para los usuarios de acuerdo a la actividad mostrada.
...
- ¿Cuál es el ID y frecuencia absoluta de la sesión más frecuente? (**NOTE-26**): t_bacf06d3-9185-4183-84ea-ff0db51475ce 888
- ¿Cuántos items tiene el itemset con mayor número de ocurrencias en la lista element length distribution? (**NOTE-27**): 20 con 1594 items
- En el contexto de este dataset con información de sesiones de streaming, ¿cuál es la interpretación de la media aritmética? (**NOTE-28**): Muestra que los usuarios entran 14.6 veces a su sesión en promedio.
...
- ¿Qué factores están asociados con que el algoritmo `apriori` con parámetros por omisión no haya producido ninguna regla de asociación, justifica tu respuesta? (**NOTE-29**):La falta de cumplimiento con los parámetros establecidos causa que ninguna regla sea creada pues no se tiene la información requerida.
- ¿Qué riesgos identificas con la asignación de valores muy bajos en los parámetros de  `support` y `confidence` al invocar el algoritmo `apriori`? Más tiempo en laejecución, pérdida de la visibilidad de los mismos pues las reglas pueden llegar a omitir ciertos datos de acuerdo a la forma en que se presentan los datos.
